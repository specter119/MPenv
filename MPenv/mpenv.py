    ...
    try:
        for command in c:
            if isinstance(command, str):
                if not os.environ.get('PIP_CERT') and not HAS_SNI and 'pip install' in command:
                    try:
                        pip_command = 'pip install --index-url=http://pypi.python.org/simple/ --trusted-host pypi.python.org ' + command.split(' ')[-1]
                        subprocess.check_call(pip_command, shell=True, executable="/bin/bash")
                    except:
