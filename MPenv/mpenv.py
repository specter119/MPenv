...
def create_env():
    ...
    parser.add_argument(
        'name', help='directory containing environment files', default=None)
    parser.add_argument(
        '--pymatpro', help='install pymatpro', action='store_true')
    parser.add_argument(
        '--conda', help='use conda instead of virtualenv', action='store_true')

    root_dir = os.getcwd()
    args = parser.parse_args()
    env_exists = os.path.exists(args.name)
    virtenv_dir = os.path.join(root_dir, args.name,
                               "virtenv_{}".format(args.name))
    virtenv_exists = os.path.exists(virtenv_dir)

    if env_exists:
        print('OK, environment directory {} exists.'.format(args.name))
        print(
            '  => Will overwrite config files and clone missing code repositories.'
        )
        print('  => FYI: Remove {} directory to start from scratch'.format(
            args.name))
        if virtenv_exists:
            print('OK, virtenv directory for {} exists.'.format(args.name))
            print(
                '  => Will NOT install codes (i.e. run `pip install -e .` in code repos).'
            )
            print('  => FYI: remove {} to reinstall codes'.format(virtenv_dir))
        else:
            print('OK, virtenv directory for {} does not exist.'.format(
                args.name))
            print(
                '  => Will install codes (i.e. run `pip install -e .` in all code repos).'
            )
    else:
    ...
    with open(BASHRC_FILE, 'r') as f:
        if re.search(r'<---MPenv {}'.format(args.name), f.read()):
            print('ERROR: make sure to remove {} section from {}!'.format(
                args.name, BASHRC_FILE))
            return
    ...
    MACHINES = (
        'Mendel', 'Vesta', 'Edison', 'Cori', 'SjtuPi'
    )  # note: you must modify BASH_template.txt when adding machines

    print('VALIDATING DIRECTORY')
    envtype = "FW"
    if not os.path.exists(files_dir):
        print(
            "ERROR: Files directory {} does not exist! Please make sure you are typing the ENVIRONMENT name and not the directory name.".
            format(files_dir))
        return
    ...
    if not virtenv_exists:
        ...
        if args.conda:
            c.append("conda create --prefix virtenv_{} numpy scipy matplotlib".
                     format(args.name))
            c.append(
                ('print',
                 'copying activate_this.py into virtualenv bin directory'))
            c.append("cp {} {}".format(
                os.path.join(static_dir, 'activate_this.py'),
                os.path.join('virtenv_{}'.format(args.name), 'bin')))
            # FIXME use_hz_mgi alias needs different source command with conda
        else:
            c.append(("mkdir", "virtenv_{}".format(args.name)))
            c.append(
                "virtualenv --no-site-packages virtenv_{}".format(args.name))

    c.append(('print', 'ACTIVATE VIRTUALENV'))
    c.append(
        ("activate",
         os.path.join(root_dir, args.name,
                      'virtenv_{}/bin/activate_this.py'.format(args.name))))
    ...
    if envtype == "MP" or envtype == "rubicon":
        ...
        if not os.path.exists(os.path.join(codes_dir, 'pymatgen')):
            ...
            if not os.path.exists(os.path.join(codes_dir, 'pymatgen-db')):
                ...
                c.append(
                    "git clone git@github.com:materialsproject/pymatgen-db.git"
                )
            ...
            if not os.path.exists(os.path.join(codes_dir, 'custodian')):
                ...
                c.append(
                    "git clone git@github.com:materialsproject/custodian.git")
            ...
            if not os.path.exists(os.path.join(codes_dir, 'MPWorks')):
                ...
                c.append(
                    "git clone git@github.com:materialsproject/MPWorks.git")
        ...
        for command in c:
            if isinstance(command, str):
                if not os.environ.get(
                        'PIP_CERT'
                ) and not HAS_SNI and 'pip install' in command:
                    try:
                        pip_command = 'pip install --index-url=http://pypi.python.org/simple/ --trusted-host pypi.python.org ' + command.split(
                            ' ')[-1]
                        subprocess.check_call(
                            pip_command, shell=True, executable="/bin/bash")
                    except:
                        easy_command = 'easy_install ' + command.split(' ')[-1]
                        subprocess.check_call(
                            easy_command, shell=True, executable="/bin/bash")
                else:
                    subprocess.check_call(
                ...
                shutil.copyfile(
                    os.path.join(files_dir, command[1]),
                    os.path.join(root_dir, args.name, 'config', 'dbs',
                                 command[1]))
            elif command[0] == 'cp_script':
                shutil.copyfile(
                    os.path.join(static_dir, command[1]),
                    os.path.join(root_dir, args.name, 'config', 'scripts',
                                 command[1]))
            elif command[0] == 'append':
                replacements = {}
                if MACHINES is 'SjtuPi':
                    replacements["ACTIVATE"] = os.path.join(
                        root_dir, args.name,
                        'activate virtenv_{}'.format(args.name))
                    replacements["DEACTIVATE"] = "source deactivate"
                else:
                    replacements["ACTIVATE"] = os.path.join(
                        root_dir, args.name,
                        'virtenv_{}/bin/activate'.format(args.name))
                    replacements["DEACTIVATE"] = "deactivate"
                replacements["CONFIG_LOC"] = os.path.join(
                    root_dir, args.name, 'config')
                replacements["ENV_LOC"] = os.path.join(root_dir, args.name)
                replacements["LOGDIR"] = os.path.join(root_dir, args.name,
                                                      'config', 'logs')
                replacements["NAME"] = args.name
                replacements[
                    "OPENBABEL"] = "export PYTHONPATH=/project/projectdirs/jcesr/openbabel/lib:$PYTHONPATH" if envtype == "rubicon" else ""
                replacements[
                    "PACKMOL"] = "export PATH=$PATH:/project/projectdirs/jcesr/packmol_hopper" if envtype == "rubicon" else ""
                replacements[
                    "MATERIALSPROJECT"] = "export VASP_PSP_DIR=/project/projectdirs/matgen/POTCARs" if envtype == "MP" else ""
                ...
                for machine in MACHINES:
                    ...
                    with open(os.path.join(static_dir, 'FW_config.yaml')) as f:
                        ...
                        appendtext = t.substitute(replacements)
                        with open(
                                os.path.join(root_dir, args.name, 'config',
                                             'config_{}'.format(machine),
                                             'FW_config.yaml'), 'w+') as f2:
                            f2.write(appendtext)

                    with open(
                            os.path.join(static_dir, 'my_fworker.yaml')) as f:
                        t = CustomTemplate(f.read())
                        appendtext = t.substitute(replacements)
                        with open(
                                os.path.join(root_dir, args.name, 'config',
                                             'config_{}'.format(machine),
                                             'my_fworker.yaml'), 'w+') as f2:
                            f2.write(appendtext)

                    with open(
                            os.path.join(static_dir, 'qadapter_{}.yaml'.format(
                                machine))) as f:
                        t = CustomTemplate(f.read())
                        appendtext = t.substitute(replacements)
                        with open(
                                os.path.join(root_dir, args.name, 'config',
                                             'config_{}'.format(machine),
                                             'my_qadapter.yaml'), 'w+') as f2:
                            f2.write(appendtext)

                    with open(
                            os.path.join(files_dir, 'my_launchpad.yaml')) as f:
                        t = CustomTemplate(f.read())
                        appendtext = t.substitute(replacements)
                        with open(
                                os.path.join(root_dir, args.name, 'config',
                                             'config_{}'.format(machine),
                                             'my_launchpad.yaml'), 'w+') as f2:
                            f2.write(appendtext)
